...
4. Select **REST Attribute Enrichment** (for OIDC clients) or **SAML REST Attribute Enrichment** (for SAML clients) from the list.
...
3. Select **REST Attribute Enrichment** (for OIDC scopes) or **SAML REST Attribute Enrichment** (for SAML scopes) and configure it.
4. Click **Save**.
...
| `endpoint.N.saml.nameformat` | Endpoint N: SAML NameFormat | *(SAML only)* The NameFormat applied to claims generated by this endpoint. |
...
| `endpoint.1.mapping` | `$.data.ldapUser.title.org→org, $.data.ldapUser.team.name→team_name, $.data.ldapUser.memberOf→groups` |
...
Or for SAML responses, the above logic directly translates to 3 attributes pushed natively to your `AttributeStatement` assertion.
